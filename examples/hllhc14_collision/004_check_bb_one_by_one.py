...
survey_strong = collider[name_strong].survey(element0=f'ip{ip_n}', reverse=True)
...
with xt.tracker._temp_knobs(collider, knobs={'beambeam_scale': 0}):
    ...
    tw_z_crab_minus = collider[name_strong].twiss(zeta0=-z_crab_test, method='4d',
                                freeze_longitudinal=True, reverse=True)
phi_crab_x = -(tw_z_crab_plus[f'ip{ip_n}', 'x'] - tw_z_crab_minus[f'ip{ip_n}', 'x']) / (2*z_crab_test)
phi_crab_y = -(tw_z_crab_plus[f'ip{ip_n}', 'y'] - tw_z_crab_minus[f'ip{ip_n}', 'y']) / (2*z_crab_test)
...
for ii, zz in list(zip(range(-(num_slices_head_on - 1) // 2,
    ...
    if tw_weak[f'ip{ip_n}', 'px'] < 1e-6:
        # Vertical crossing
        assert np.isclose(ee_weak.alpha, np.pi/2, atol=1e-3, rtol=0)
        assert np.isclose(
            2*ee_weak.phi, tw_weak[f'ip{ip_n}', 'py'] - tw_strong[f'ip{ip_n}', 'py'],
            atol=1e-7, rtol=0)
    else:
        assert np.isclose(ee_weak.alpha, 0, atol=1e-3, rtol=0)
        assert np.isclose(
            2*ee_weak.phi, tw_weak[f'ip{ip_n}', 'px'] - tw_strong[f'ip{ip_n}', 'px'],
            atol=1e-7, rtol=0)
